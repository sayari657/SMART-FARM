...
class ChatMessage(Base):
    """Messagerie ferme : discussion entre le propriétaire et ses ouvriers.
    Les pièces jointes (image/vidéo) sont stockées en data-URL base64,
    comme WorkerReport.photo_b64."""
    __tablename__ = "chat_messages"

    id              = Column(Integer, primary_key=True, index=True)
    farm_id         = Column(Integer, ForeignKey("farms.id", ondelete="CASCADE"), nullable=False, index=True)
    sender_id       = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    sender_role     = Column(String(20), nullable=True)    # owner | worker
    sender_name     = Column(String(120), nullable=True)
    text            = Column(Text, nullable=True)
    attachment_b64  = Column(Text, nullable=True)          # data URL (image/* ou video/*)
    attachment_type = Column(String(16), nullable=True)    # image | video
    attachment_name = Column(String(200), nullable=True)
    created_at      = Column(DateTime, default=datetime.utcnow, index=True)

    __table_args__ = (Index("ix_chat_farm_created", "farm_id", "created_at"),)
...
# ---------------------------------------------------------------------------
# Weather alerts (farm-scoped, météo-driven → email + push + WhatsApp)
# ---------------------------------------------------------------------------

class WeatherAlert(Base):
    """A weather-driven alert for a farm (heat, frost, storm, heavy rain, UV…).

    Generated by weather_alert_service from live Open-Meteo data and dispatched
    to the farm owner + assigned workers by email (and other channels).
    """
    __tablename__ = "weather_alerts"

    id           = Column(Integer, primary_key=True, index=True)
    farm_id      = Column(Integer, ForeignKey("farms.id", ondelete="CASCADE"), nullable=False, index=True)
    alert_type   = Column(String(30),  nullable=False)   # heat | frost | storm | rain | uv
    severity     = Column(String(20),  default="warning")  # info | warning | critical
    title        = Column(String(160), nullable=False)
    message      = Column(Text,        nullable=False)
    emoji        = Column(String(10),  nullable=True)
    metric       = Column(String(40),  nullable=True)    # e.g. "temperature_2m_max"
    value        = Column(Float,       nullable=True)     # observed/forecast value
    threshold    = Column(Float,       nullable=True)     # threshold that was crossed
    unit         = Column(String(16),  nullable=True)     # °C | km/h | mm | UV
    is_resolved  = Column(Boolean,     default=False)
    email_sent   = Column(Boolean,     default=False)
    notified_count = Column(Integer,   default=0)         # nb of recipients reached
    created_at   = Column(DateTime,    default=datetime.utcnow, index=True)
    resolved_at  = Column(DateTime,    nullable=True)
    ...
    __table_args__ = (
        Index("ix_weather_alert_farm_resolved", "farm_id", "is_resolved"),
        Index("ix_weather_alert_type", "farm_id", "alert_type", "is_resolved"),
    )
