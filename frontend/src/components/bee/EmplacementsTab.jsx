  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
           <h1 style={{ fontSize: 32, fontWeight: 800, color: 'white' }}>Sites & Emplacements</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 24 }}>
        {emplacements.length === 0 ? (
          ...
          >
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: 'white', marginBottom: 6 }}>{e.name || e.nom}</div>
            ...
            <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                 ...
                 <span style={{ color: 'white', fontWeight: 700, fontSize: 14 }}>{e.flower_type || e.typeFleur}</span>
              </div>
              <div style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.05)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                 <LayoutGrid size={14} color={COLORS.accent} />
                 <span style={{ color: 'white', fontWeight: 800, fontSize: 13 }}>Accéder aux ruches</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: COLORS.surface, width: 500, borderRadius: 32, border: `1px solid ${COLORS.border}`, overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
            <div style={{ padding: '24px 32px', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ color: 'white', fontSize: 22, fontWeight: 900 }}>Configuration du Site</h2>
              <button onClick={() => setModalActive(null)} style={{ background: 'rgba(255,255,255,0.05)', border: 'none', color: 'white', cursor: 'pointer', width: 40, height: 40, borderRadius: '50%' }}><X size={20} /></button>
            </div>
            ...
            <div style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 20 }}>
                 <div>
                    ...
                    <input value={empForm.name} onChange={(e) => setEmpForm({ ...empForm, name: e.target.value })} placeholder="ex: Parc Nord" style={{ width: '100%', height: 50, background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '0 16px', color: 'white', fontSize: 15 }} />
                 </div>
                 <div>
                    <label style={{ color: COLORS.textMuted, fontSize: 12, fontWeight: 800, marginBottom: 8, display: 'block' }}>SAISON</label>
                    <select value={empForm.season} onChange={(e) => setEmpForm({ ...empForm, season: e.target.value })} style={{ width: '100%', height: 50, background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '0 16px', color: 'white' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                 <div>
                    ...
                    <input value={empForm.region} onChange={(e) => setEmpForm({ ...empForm, region: e.target.value })} placeholder="ex: Bizerte" style={{ width: '100%', height: 50, background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '0 16px', color: 'white', fontSize: 15 }} />
                 </div>
                 <div>
                    <label style={{ color: COLORS.textMuted, fontSize: 12, fontWeight: 800, marginBottom: 8, display: 'block' }}>TYPE DE FLORAISON</label>
                    <input value={empForm.flower_type} onChange={(e) => setEmpForm({ ...empForm, flower_type: e.target.value })} placeholder="ex: Oranger, Thym..." style={{ width: '100%', height: 50, background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '0 16px', color: 'white' }} />
                 </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <input value={empForm.latitude} onChange={(e) => setEmpForm({ ...empForm, latitude: e.target.value })} placeholder="Lat" style={{ height: 50, background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '0 16px', color: 'white' }} />
                  <input value={empForm.longitude} onChange={(e) => setEmpForm({ ...empForm, longitude: e.target.value })} placeholder="Lng" style={{ height: 50, background: 'rgba(255,255,255,0.02)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '0 16px', color: 'white' }} />
                </div>
              </div>
