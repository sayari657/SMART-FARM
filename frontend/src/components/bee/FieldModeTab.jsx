...
const HEALTH_OPTS = [
  ...
  { id: 'warning',   label: 'Surveiller', icon: AlertTriangle, color: COLORS.honey },
  ...
  return (
    <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 20, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        <button onClick={dec}
          style={{ width: 52, height: 52, borderRadius: 16, background: 'rgba(0,0,0,0.06)', border: `1px solid ${COLORS.border}`, color: COLORS.text, fontSize: 26, fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1, userSelect: 'none', WebkitUserSelect: 'none' }}>
          −
        </button>
      ...
      {/* Mini bar */}
      {max !== undefined && (
        <div style={{ width: '100%', height: 4, borderRadius: 4, background: 'rgba(0,0,0,0.08)' }}>
    ...
    <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.success}40`, borderRadius: 18, padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {Object.entries(upd).length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {Object.entries(upd).map(([k, v]) => (
            <div key={k} style={{ background: 'rgba(0,0,0,0.05)', border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: '6px 12px', fontSize: 11 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={() => { setApplied(true); }}
          style={{ height: 48, padding: '0 18px', borderRadius: 14, background: COLORS.surface, border: `1px solid ${COLORS.border}`, color: COLORS.textMuted, cursor: 'pointer', fontSize: 13 }}>
          Ignorer
        </button>
      </div>
    </div>
...
export default function FieldModeTab({ hive, onVisitCreated, toast }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    // Preview
    const rPrev = await beeApi.previewVisit(payload);
    const prevData = rPrev.ok ? await rPrev.json() : null;

    // Save
    const rSave = await beeApi.createVisit(payload);
    if (rSave.ok) {
  ...
  const gc = healthState === 'health' ? COLORS.success : healthState === 'warning' ? COLORS.honey : healthState === 'urgent' ? COLORS.error : COLORS.info;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {HEALTH_OPTS.map(opt => {
            ...
            return (
              <button key={opt.id} onClick={() => setHealthState(opt.id)}
                style={{ height: 64, borderRadius: 16, background: active ? opt.color + '25' : COLORS.surface, border: `2px solid ${active ? opt.color : COLORS.border}`, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.15s', transform: active ? 'scale(1.03)' : 'scale(1)' }}>
      ...
      <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 18, padding: '14px 18px' }}>
        ...
        <textarea
          ...
          rows={2}
          style={{ width: '100%', background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '10px 14px', color: COLORS.text, fontSize: 13, resize: 'none', outline: 'none', fontFamily: 'inherit' }}
        />
      </div>

      {/* Save button */}
      <button onClick={handleSave} disabled={saving}
        style={{ height: 60, borderRadius: 18, background: saving ? 'rgba(0,0,0,0.06)' : `linear-gradient(135deg, ${gc}, ${gc}bb)`, border: `2px solid ${saving ? COLORS.border : gc + '60'}`, color: saving ? COLORS.textMuted : 'white', fontWeight: 900, fontSize: 16, cursor: saving ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, boxShadow: saving ? 'none' : `0 8px 24px -4px ${gc}40`, transition: 'all 0.2s' }}>
        {saving
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 10 }}>
        {[
          ...
          <div key={s.label} style={{ background: 'rgba(0,0,0,0.03)', border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '10px 14px', textAlign: 'center' }}>
