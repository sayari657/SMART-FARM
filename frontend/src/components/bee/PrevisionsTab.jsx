...
const STATUS_CONFIG = {
  ...
  doing: { label: 'EN COURS',  color: COLORS.honey,        bg: 'rgba(251,191,36,0.15)' },
...
const TaskIcon = ({ status }) => {
  ...
  if (status === 'doing') return <Clock size={16} color={COLORS.honey} />;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 30, fontWeight: 900, color: COLORS.text, margin: 0 }}>Feuilles de Route</h1>
        ...
        {totalMissions > 0 && (
          <div style={{ display: 'flex', gap: 16 }}>
            <div style={{ padding: '8px 18px', background: COLORS.surface, borderRadius: 12, border: `1px solid ${COLORS.border}` }}>
              ...
              <span style={{ color: COLORS.text, fontWeight: 900 }}>{totalMissions}</span>
            </div>
            <div style={{ padding: '8px 18px', background: COLORS.surface, borderRadius: 12, border: `1px solid ${COLORS.border}` }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 28 }}>
        ...
        <div style={{ background: COLORS.surface, borderRadius: 28, border: `1px solid ${COLORS.border}`, padding: 28, height: 'fit-content' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {/* Besoins estimés */}
            {form.rucheId && (
              <div style={{ background: 'rgba(0,0,0,0.03)', borderRadius: 12, padding: '12px 16px', border: `1px solid ${COLORS.border}` }}>
                <div style={{ color: COLORS.textMuted, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
                  {loadingNeeds ? '⏳ Estimation IA...' : '📦 Besoins estimés'}
                </div>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {Object.entries(predictedNeeds).map(([k, v]) => (
                    <span key={k} style={{ fontSize: 11, background: `${COLORS.accent}18`, color: COLORS.accent, padding: '3px 10px', borderRadius: 20, fontWeight: 700 }}>
                      {k === 'sirop' ? `${v}L Sirop` : k === 'pate' ? `${v}kg Pâte` : k === 'traitement' ? `${v} Trait.` : `${v} Cadres`}
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 18 }}>
              ...
              {taskList.length > 0 && (
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {taskList.map((t, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.03)', padding: '8px 14px', borderRadius: 10, border: `1px solid ${COLORS.border}` }}>
            ...
            <button
              ...
              style={{ width: '100%', height: 54, background: `linear-gradient(135deg, ${COLORS.accent}, ${COLORS.accentDark})`, border: 'none', borderRadius: 14, color: 'white', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontSize: 14, boxShadow: `0 6px 20px ${COLORS.accent}40`, transition: 'transform 0.2s' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {previsions.length === 0 ? (
            <div style={{ height: 320, background: 'rgba(0,0,0,0.02)', border: `2px dashed ${COLORS.border}`, borderRadius: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: COLORS.textMuted, gap: 14 }}>
            ...
            return (
              <div key={p.id} style={{ background: COLORS.surface, borderRadius: 24, border: `1px solid ${COLORS.border}`, overflow: 'hidden' }}>
                ...
                {/* Mission header */}
                <div style={{ padding: '18px 24px', background: 'rgba(0,0,0,0.02)', borderBottom: `1px solid ${COLORS.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ padding: '4px 10px', borderRadius: 8, background: progress === 100 ? `${COLORS.success}18` : 'rgba(251,191,36,0.15)', border: `1px solid ${progress === 100 ? COLORS.success + '40' : 'rgba(251,191,36,0.3)'}` }}>
                      <span style={{ fontSize: 11, fontWeight: 800, color: progress === 100 ? COLORS.success : COLORS.honey }}>
                    ...
                    return (
                      <div key={task.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 14, background: cfg.bg, border: `1px solid ${COLORS.border}`, transition: 'all 0.15s' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <span style={{ color: task.status === 'done' ? COLORS.textMuted : COLORS.text, textDecoration: task.status === 'done' ? 'line-through' : 'none', fontSize: 13, fontWeight: 600 }}>
                            {task.text}
                          </span>
                        </div>
                          ...
                          {['todo', 'doing', 'done'].map(st => (
                            <button
                              ...
                              style={{
                                ...
                                color: task.status === st ? (st === 'doing' ? '#000' : COLORS.text) : COLORS.textMuted,
                ...
                {/* Logistics reminder */}
                {p.needs && (
                  <div style={{ margin: '0 24px 20px', display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 14, background: 'rgba(0,0,0,0.02)', border: `1px solid ${COLORS.border}` }}>
