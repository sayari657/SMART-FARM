...
import { simInit, step as simStep, clockUpdate as simClock } from '../lib/iotSim';
...
function IoTSimulator() {
  ...
  useEffect(() => { simClock(s.current, pushEvent); setSnap({ ...s.current }); }, []); // eslint-disable-line
  useEffect(() => {
    const iv = setInterval(() => {
      if (running) simStep(s.current, pushEvent);
      else simClock(s.current, pushEvent);    // keep the clock live even on pause
      setSnap({ ...s.current });
