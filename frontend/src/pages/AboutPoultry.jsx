...
export default function AboutPoultry() {
  ...
  const [poultryUnits, setPoultryUnits] = useState([]);
  ...
  useEffect(() => {
    animalsAPI.list({ species: 'poultry' })
      .then(res => {
        setPoultryUnits(res.data);
  ...
  const fetchUnitTelemetry = (unitId) => {
    telemetryAPI.getLatest(unitId).then(res => {
      ...
      if (res.data && res.data.metrics) {
        setStats(prev => ({
          ...prev,
          avgTemp: res.data.metrics.temperature || 32.2,
          eggYield: res.data.metrics.egg_yield || 450,
          waterLevel: res.data.metrics.water_level || 85,
          feedLevel: res.data.metrics.feed_level || 78
        }));
      }
  ...
  useEffect(() => {
    const fetchDetections = () => {
      cvAPI.recent(10).then(res => {
        const poultryRelevant = res.data.filter(d => 
          ['chicken', 'hen', 'rooster', 'poultry'].includes(d.object_class?.toLowerCase())
        );
        setRealDetections(poultryRelevant);
  ...
  return (
    <>
      <Navbar 
        title="Poultry Farm Intelligence" 
        subtitle="Automated environmental and health monitoring for poultry units" 
      />
      ...
      <div className="page-content">
        ...
        <div className="grid-2-1" style={{ marginBottom: 32, gap: 24 }}>
          <div className="card" style={{ padding: 0, overflow: 'hidden', height: '400px', display: 'flex', flexDirection: 'column' }}>
            {sourceMode === 'live' ? (
              <div style={{ flex: 1, position: 'relative', background: 'linear-gradient(135deg, #ecfeff 0%, #cffafe 100%)' }}>
                <ThreeSpeciesCard sp="poultry" count={poultryUnits.length} emoji="🐔" color="#0891b2" isActive={true} onClick={() => {}} />
                <div style={{ position: 'absolute', top: 24, left: 24, zIndex: 10 }}>
                  <h2 style={{ fontSize: 24, color: '#164e63', marginBottom: 4 }}>Gallus Gallus</h2>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <span className="badge badge-success">Optimized Temp</span>
                    <span className="badge badge-info">{poultryUnits.length} Units Active</span>
                  </div>
                </div>
              </div>
            ) : (
                ...
                {sourceMode === 'camera' && (
                  <>
                    ...
                    <button className="btn btn-primary" onClick={scanCameraFrame} disabled={isProcessing} style={{ position: 'absolute', bottom: 20, zIndex: 100, display: 'flex', alignItems: 'center', gap: 8 }}>
                      {isProcessing ? 'Analyzing...' : 'Identify Poultry (YOLO)'}
                    </button>
                  </>
                )}
                {sourceMode === 'upload' && (
            ...
            <div style={{ background: 'var(--color-surface)', padding: '12px 20px', display: 'flex', gap: 20, borderTop: '1px solid var(--color-border)' }}>
              {['Layer', 'Broiler', 'Chicks'].map(c => (
                <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '12px', fontWeight: 600 }}>
                  <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#0891b2' }} />{c}
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <ThreeTile><KPIBox icon={Thermometer} value={stats.avgTemp} label="Avg Coop Temp" colorClass="yellow" unit="°C" /></ThreeTile>
            <ThreeTile><KPIBox icon={Egg} value={stats.eggYield} label="Daily Egg Yield" colorClass="white" unit="pc" /></ThreeTile>
            <ThreeTile><KPIBox icon={Droplets} value={stats.waterLevel} label="Water Supply" colorClass="blue" unit="%" /></ThreeTile>
          </div>
        </div>

        <div className="grid-2" style={{ marginBottom: 32, gap: 24 }}>
          <div className="card">
            <div className="card-header"><div className="card-title">Environmental Parameters</div></div>
            <div style={{ padding: 20 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  { l: 'Ventilation', v: 'Excellent', p: 92, icon: Wind },
                  { l: 'Feed Level', v: `${stats.feedLevel}% Full`, p: stats.feedLevel, icon: Utensils },
                  { l: 'Ammonia', v: 'Low (Safe)', p: 15, icon: AlertCircle }
                ].map(item => (
                  <div key={item.l} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 600 }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><item.icon size={14} color="#0891b2" />{item.l}</span>
                      <span>{item.v}</span>
                    </div>
                    <div style={{ height: 4, background: 'var(--color-bg)', borderRadius: 2 }}>
                      <div style={{ width: `${item.p}%`, height: '100%', background: '#0891b2', borderRadius: 2 }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="card">
            <div className="card-header"><div className="card-title">Monitored Poultry Units</div></div>
            <div className="table-wrap">
              <table>
                <thead><tr><th>Unit</th><th>ID</th><th>Stock</th><th>Status</th></tr></thead>
                <tbody>
                  {poultryUnits.map(unit => (
                    <tr key={unit.id}>
