  ...
  return (
    <div style={{ position: 'relative' }}>
      <button
        onClick={() => setOpen(o => !o)}
        disabled={loading}
        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 6, cursor: 'pointer', color: '#64748b', fontSize: 12 }}
      >
        <PlanBadge plan={currentPlan} />
        <ChevronDown size={12} />
      </button>
      {open && (
        <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: 4, background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 8, zIndex: 50, minWidth: 130, overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,.1)' }}>
          {['free', 'pro', 'enterprise'].map(p => (
            <button key={p} onClick={() => change(p)}
              style={{ display: 'block', width: '100%', padding: '8px 14px', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', color: p === currentPlan ? '#1e293b' : '#64748b', fontSize: 12 }}
            >
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1e293b', margin: 0 }}>Tenants — Propriétaires</h1>
          <p style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>{tenants.length} comptes enregistrés</p>
        </div>
        <button onClick={load} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 8, color: '#64748b', cursor: 'pointer', fontSize: 13 }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 200, position: 'relative' }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '9px 12px 9px 34px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, color: '#1e293b', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {PLANS.map(p => (
            <button key={p} onClick={() => setFilter(p)}
              style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid', borderColor: filter === p ? PLAN_COLORS[p] || '#7c3aed' : '#e2e8f0', background: filter === p ? `${PLAN_COLORS[p] || '#7c3aed'}15` : 'transparent', color: filter === p ? '#1e293b' : '#64748b', fontSize: 12, fontWeight: 600, cursor: 'pointer', textTransform: 'uppercase' }}
            >
              {p}
            </button>
          ))}
            ...
            {loading ? (
              ...
              >
                <td style={{ padding: '14px 16px' }}>
                  <div style={{ fontWeight: 600, fontSize: 13, color: '#1e293b' }}>{t.full_name || t.username}</div>
                  <div style={{ fontSize: 11, color: '#64748b' }}>@{t.username}</div>
                </td>
                <td style={{ padding: '14px 16px' }}>
                  <div style={{ fontSize: 12, color: '#475569' }}>{t.email || '—'}</div>
                ...
                <td style={{ padding: '14px 16px', fontSize: 13, color: '#475569' }}>{t.animal_count}</td>
                <td style={{ padding: '14px 16px' }}>
