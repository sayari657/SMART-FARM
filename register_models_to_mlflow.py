...
try:
    ...
    # Fallback: resolve paths relative to this script — works on any OS/machine
    _ROOT = Path(__file__).resolve().parent
    class Settings:
        YOLO_BEE_PATH     = str(_ROOT / "ai_assets" / "animal_weights" / "bee" / "final_export" / "best.pt")
        YOLO_GOAT_PATH    = str(_ROOT / "ai_assets" / "animal_weights" / "model goat cow" / "best.pt")
        YOLO_LEAVES_PATH  = str(_ROOT / "ai_assets" / "plantations" / "Detection diseases Leaves" / "best.pt")
        YOLO_OLIVE_PATH   = str(_ROOT / "ai_assets" / "plantations" / "model olive-tree-diseases" / "best.pt")
        YOLO_INSECTS_PATH = str(_ROOT / "ai_assets" / "plantations" / "model insects_final" / "best.pt")
        YOLO_FIRE_PATH    = str(_ROOT / "ai_assets" / "Alert" / "model-fire-detection-and-smoke" / "best.pt")
    settings = Settings()

# MLflow URI from env — consistent across all scripts
    ...
    for model_name, path in MODELS_TO_REGISTER.items():
        ...
        try:
            with mlflow.start_run(run_name=f"Ingestion_{model_name}"):
                ...
                if os.path.exists(args_path):
                    try:
                        with open(args_path, 'r') as f:
                            ...
                            important_keys = ['model', 'data', 'epochs', 'patience', 'batch',
                                              'imgsz', 'save', 'device', 'optimizer', 'lr0', 'weight_decay']
                ...
                # 6. Register in Model Registry and promote to Staging
                run_id = mlflow.active_run().info.run_id
                try:
                    client.create_registered_model(model_name)
                except Exception:
                    pass  # Model already registered — not an error
